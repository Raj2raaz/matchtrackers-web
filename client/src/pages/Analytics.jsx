...
<div className="w-full order-3 md:order-1 lg:w-1/4">
  {/* Recent Highlights */}
  <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg mb-6 border border-gray-200 dark:border-gray-700 p-5">
      ...
      {isLoading ? (
        <div className="animate-pulse space-y-2">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-8 bg-gray-200 dark:bg-gray-700 rounded-full"
            ></div>
          ))}
        </div>
      ) : content === "cricket" ? (
        recentMatches && recentMatches.length > 0 ? (
          recentMatches.slice(0, noOfRecentMatches).map((e, i) =>
            e.seriesAdWrapper ? (
              <div
                key={i}
                className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors cursor-pointer flex justify-between items-center mt-2 pl-4 pr-3 py-2 rounded-full"
              >
                <p className="truncate max-w-[14rem] overflow-hidden whitespace-nowrap font-medium dark:text-gray-200">
    ...
    {!isLoading && recentMatches && recentMatches.length > 0 && (
      <button
        ...
        className="flex items-center justify-center w-full mt-4 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
      >
        {noOfRecentMatches >= recentMatches.length
          ? "See Less"
          : "See More"}{" "}
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 border border-gray-200 dark:border-gray-700">
    ...
    <div className="relative mb-4">
      <input
        ...
        className="w-full bg-gray-50 dark:bg-gray-700 p-3 pl-10 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:placeholder-gray-400 dark:text-gray-200"
      />
      <BsSearch className="absolute left-3 top-3.5 text-gray-400 dark:text-gray-500" />
    </div>
    <ul className="max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-gray-100 dark:scrollbar-track-gray-700">
      {filteredCompetitions.map((country, index) => (
        <li
          key={index}
          className="flex items-center justify-between py-3 px-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer transition-colors border-b border-gray-100 dark:border-gray-700 last:border-0"
        >
...
<div className="w-full order-1 md:order-2 lg:w-2/5">
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
    {/* Tabs and filters */}
    <div>
      <div className="flex items-center border-b border-gray-200 dark:border-gray-700 px-2 pt-2">
        <button
    ...
    {activeTab === "All" ? (
      <div>
        <div className="px-4 pb-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex flex-wrap gap-2 mt-4">
            {["league", "international", "domestic", "women"].map((section) => (
              <button
                key={section}
          ...
          <div className="flex mt-4">
            <button
              className={`px-5 py-1.5 rounded-full transition-colors mx-1 ${
                matchType === "Live"
                  ? "bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-300 font-medium"
          ...
          {isLoading ? (
            ...
            getVisibleMatches.map((matchData, idx) => {
              ...
              return (
                <div
                  key={`match-${id}`}
                  className={`p-4 hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors cursor-pointer ${
                    isSelected ? "bg-blue-50 dark:bg-blue-900" : ""
                  }`}
                  onClick={() => handleMatchSelect(match, id)}
                >
                  <div className="text-xs flex justify-between items-center text-gray-500 dark:text-gray-400 mb-2 font-medium relative">
                    <p>{seriesName}</p>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate("/cricket/analytics/match/" + match.matchInfo.matchId);
                      }}
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                    {match.matchInfo.matchDesc} • {match.matchInfo.venueInfo.ground},{" "}
                    {match.matchInfo.venueInfo.city}
                  </div>

                  {/* Teams */}
                  <div className="space-y-4">
                    {[match.matchInfo.team1, match.matchInfo.team2].map((team, i) => (
                      <div key={i} className="flex justify-between items-center">
                        <div className="font-medium flex items-center gap-3 text-gray-900 dark:text-gray-200">
                          <Image
                        ...
                        {match.matchScore?.[i === 0 ? "team1Score" : "team2Score"]?.inngs1 ? (
                          <div className="font-medium text-gray-900 dark:text-gray-200">
                            {match.matchScore[i === 0 ? "team1Score" : "team2Score"].inngs1.runs}-
                            {match.matchScore[i === 0 ? "team1Score" : "team2Score"].inngs1.wickets}
                            <span className="text-gray-500 dark:text-gray-400 text-sm ml-1">
                              ({match.matchScore[i === 0 ? "team1Score" : "team2Score"].inngs1.overs} ov)
                            </span>
                          </div>
                        ) : (
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        addMatchToFavorites(match.matchInfo.matchId, {
                          seriesName,
                      ...
                      className={`p-2 cursor-pointer transition-colors ${
                        userData?.favorites?.matches?.some(
                          (fmatch) => fmatch.matchId === match.matchInfo.matchId
                        )
                          ? "text-yellow-500"
      ...
      <div className="h-[30rem] divide-y divide-gray-300 dark:divide-gray-700">
        {userData?.favorites?.matches?.length > 0 ? (
              ...
              >
                <div className="text-xs flex justify-between items-center text-gray-500 dark:text-gray-400 mb-2 font-medium relative">
                  ...
                  <button
                    onClick={(el) => {
                      el.stopPropagation();
                      navigate("/cricket/analytics/match/" + e.matchId);
                    }}
                    className="bg-secondary text-base text-white px-3 py-0.5 rounded"
                  >
                    View Match
                ...
                <div className="space-y-4">
                  {[match?.imgs[0], match?.imgs[1]].map((img, idx) => (
                    <div key={idx} className="flex justify-between items-center">
                      <div className="font-medium flex items-center gap-3 text-gray-900 dark:text-gray-200">
                        <Image
                          ...
                          className="rounded-full h-8 w-8 object-cover shadow-sm"
                        />
                        <span>{idx === 0 ? match?.team1Name : match?.team2Name}</span>
                      </div>
                      <span className="text-gray-400">-</span>
                    </div>
                  ))}
          ...
          <p className="text-center my-10 text-gray-500 dark:text-gray-400">No Matches Found</p>
        )}
      </div>
    )}
  </div>
          ...
          <div className="w-full order-2 md:order-3 lg:w-1/3">
    ...
    {isOddsLoading ? (
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <TimeDisplay time={matchOdds.matchTime} />
        </div>

        {matchOdds.markets.map((market, idx) => (
          <div
            key={idx}
            className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow p-3"
          >
            <div className="flex items-center space-x-2 mb-2">
              <img src={market.logo} alt="" className="w-4 h-4" />
            ...
            <div className="grid grid-cols-3 gap-2">
              {market.choices.map((choice, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded p-2 text-center hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer"
                >
      ...
      <div className="p-4 text-center text-gray-500 dark:text-gray-400">
        ...
        <p className="mt-2">Select a match to view odds and predictions</p>
      </div>
    )}
  </div>
</div>
