...
export default function Home() {
  ...
  <img
    src={header}
    alt="Hero"
    className="w-full h-auto transform transition-transform duration-700 hover:scale-105 animate-float"
  />
  ...
  <div className="absolute bottom-0 h-[60%] bg-gradient-to-t from-[#002B5B] via-[#002B5B]/80 to-transparent w-full z-20">
    <div className="md:flex flex-col hidden w-full gap-4 justify-end text-white py-8 h-full items-center">
      ...
      {/* Feature Buttons */}
      <div className="md:flex hidden gap-4 flex-wrap justify-center animate-fadeInUp opacity-0"
      ...
      >
        {[
          ...
          <button
            ...
            key={i}
          >
            <span className="mr-2 group-hover:animate-bounce inline-block">{item.icon}</span>
            {item.name}
          </button>
        ))}
      </div>
  ...
  <div className="absolute inset-0 opacity-10">
    <div className="absolute top-10 left-10 w-20 h-20 sm:w-32 sm:h-32 bg-white rounded-full animate-float"></div>
    <div
      className="absolute bottom-10 right-10 w-14 h-14 sm:w-20 sm:h-20 bg-blue-300 rounded-full animate-float"
      style={{ animationDelay: "1s" }}
    ></div>
    <div
      className="absolute top-1/2 left-1/4 w-12 h-12 sm:w-16 sm:h-16 bg-purple-300 rounded-full animate-float"
      ...
      topSectionMatches.map((match, i) => (
        ...
        >
          ...
          <div
            ...
            className="match-card bg-white/95 backdrop-blur-sm relative cursor-pointer shadow-xl mt-4 rounded-2xl border border-white/20 p-4 sm:p-6 hover:shadow-2xl group overflow-hidden"
          >
            ...
            <div className="flex flex-col sm:flex-row text-sm items-start sm:items-center justify-between gap-2 sm:gap-0 relative z-10">
              <div className="space-y-1">
                <div className="flex items-center flex-wrap gap-2">
                  <p className="font-semibold text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded-full">
                ...
                <p className="text-xs truncate max-w-[220px] text-gray-500 font-medium">
            ...
            <div className="flex justify-between items-center mt-6 gap-3 sm:gap-6 relative z-10">
              ...
              <div className="flex flex-col flex-1 items-start space-y-2 relative">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 px-2 py-1 rounded-lg">
              ...
              <div className="text-center space-y-2">
                <p className="font-bold text-xl sm:text-2xl text-gray-400 animate-pulse">
                  vs
                </p>
                <div className="bg-gradient-to-r from-gray-100 to-gray-200 rounded-full px-3 py-1 shadow-inner">
              ...
              <div className="flex flex-col flex-1 items-end space-y-2 relative">
                ...
                <div className="bg-gradient-to-r from-purple-50 to-blue-50 px-2 py-1 rounded-lg">
                  <p className="text-sm font-semibold text-gray-700">
...
<div className="flex flex-col md:flex-row gap-8 my-8 px-5 md:px-24">
  ...
  <div className="w-full md:w-1/3 flex flex-col">
    ...
    <div className="overflow-y-auto space-y-4 pr-1 scrollbar-hide" style={{ maxHeight: "28rem" }}>
      {seriesList && seriesList.length > 0 ? (
        seriesList.slice(0, seriesShown).map((e, i) => (
          <div
            onClick={() => navigate("/cricket/schedules/" + e.id)}
            key={i}
            className="bg-gray-100 hover:bg-gray-200 cursor-pointer w-full px-6 py-5 rounded-xl shadow-sm border border-gray-300 transition duration-200"
          >
            <p className="text-xl text-gray-800 font-semibold leading-snug break-words">
              {e.name}
            </p>
          </div>
        ))
      ) : (
        <p className="text-gray-500 italic text-lg">No recent matches available</p>
      )}
    </div>
  </div>
