...
export default function Analytics() {
  ...
  // Helper function to get icon based on bet name
  const getBetIcon = (betName) => {
    switch (betName) {
      case "Match Winner":
        return <Award className="text-yellow-400" size={18} />;
      case "Double Chance":
  ...
  // Check if odds data exists and has bookmakers
  const bookmakers = odds?.[0]?.bookmakers || [];
  const totalItems =
    bookmakers[0]?.bets?.length > 0 ? bookmakers[0]?.bets?.length : 0;
        ...
        <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div className="p-4">
                        <div className="flex flex-col md:flex-row items-center justify-between">
                          ...
                          <div className="flex flex-col items-center justify-center w-full md:w-2/12 mx-4 mb-3 md:mb-0">
                            ...
                            <span
                              className={`text-xs text-nowrap px-3 py-1 rounded-full ${
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-4 rounded-t-xl">
              <div className="flex justify-between items-center">
                <h2 className="text-white font-semibold text-xl">Odds</h2>
                <div className="text-white text-sm">
                  {odds?.[0]?.league?.name} - {odds?.[0]?.league?.country}
                </div>
              </div>
            </div>
                ...
                {bookmakers.length > 0 ? (
                  <div>
                    {bookmakers[0].bets &&
                      bookmakers[0].bets
                        ...
                        .map((bet, betIndex) => (
                          <div
                            key={betIndex}
                            className="bg-white rounded-xl shadow-md p-4 mb-4 border-l-4 border-blue-500 hover:shadow-lg transition duration-300"
                          >
                    ...
                    {visibleItems < totalItems && (
                      <button
                        onClick={loadMore}
                        className="w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2.5 px-4 rounded-lg flex items-center justify-center"
                      >
                        <span>See More</span>
                    ...
                    {visibleItems > 10 && (
                      <button
                        onClick={showLess}
                        className="w-full mt-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-2.5 px-4 rounded-lg flex items-center justify-center"
                      >
                        <span>Show Less</span>
